...
window.addEventListener('DOMContentLoaded', async function(event) {
    ...
    for (let i=0; i<movies.length; i++){
      let movieResults = movies[i].results  
      for (let a=0; a<movieResults.length; a++){
        let movieId = movieResults[a].id
        let moviePoster = movieResults[a].poster_path
        console.log(movieId)
    ...
    for (let i=0; i<movies.length; i++){
      ...
      for (let a=0; a<movieResults.length; a++){
        let movieId = movieResults[a].id
        let movieHistory = document.querySelector(`.movie-${movieId}`).addEventListener('click', function(event){
          event.preventDefault()
    ...
    let watchSnapshot = await db.collection('hw6').get()
    let watchedMovies = watchSnapshot.docs
    for (let c=0; c<watchedMovies.length; c++){
      let watchedId = watchedMovies[c].id
      let watchedData = watchedMovies[c].data()
      let watchedDiv = watchedData.text
      console.log(watchedDiv)

      let docRef = await db.collection('hw6').add({
        id: watchedId
